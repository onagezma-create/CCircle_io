...
const BOT_START_POSITIONS = [
  {x:12,y:12},{x:50,y:10},{x:88,y:12},{x:10,y:50},{x:90,y:50},
  {x:12,y:88},{x:50,y:90},{x:88,y:88},{x:28,y:28},{x:72,y:28},
  {x:28,y:72},{x:72,y:72},
];
...
function startRoom(room) {
  ...
  const botsNeeded = Math.max(0, BOTS_TO_FILL - totalReal);
  ...
  for (let i = 0; i < botsNeeded; i++) {
    ...
    let posIdx = i % BOT_START_POSITIONS.length;
    const p = {
      x: Math.max(6, Math.min(COLS-6, BOT_START_POSITIONS[posIdx].x + Math.floor((Math.random()-.5)*8))),
      y: Math.max(6, Math.min(ROWS-6, BOT_START_POSITIONS[posIdx].y + Math.floor((Math.random()-.5)*8))),
    };
    room.bots.set(botId, makeEntity(botId, p.x, p.y, name, c.color, c.trail, true, room.ownerGrid));
  }
...
function onCellEnter(e, room) {
  ...
  if (trailOwner >= 0) {
    if (trailOwner === e.id) {
      killEntity(e, null, room); // наехал на свой след → смерть
    } else {
      // Наехал на чужой след → ХОЗЯИН следа умирает
      const owner = findEntity(room, trailOwner);
...
function buildDelta(room) {
  // Only send positions + trails each tick (not full grids)
  const positions = {};
  const trails    = {};
  const deaths    = [];

  for (const e of room.allEntities()) {
    positions[e.id] = { px: Math.round(e.px), py: Math.round(e.py), alive: e.alive };
    trails[e.id]    = [...e.trail];
  }
  return {
    ...
    positions,
    trails,
    ownerDiff: [],  // territory diffs sent separately on capture
  };
}
...
function tryMatchmaking() {
  ...
  const playerPositions = [
    {x:50,y:50}, {x:15,y:15}, {x:85,y:15}, {x:15,y:85},
    {x:85,y:85}, {x:50,y:15}, {x:50,y:85}, {x:15,y:50},
    {x:85,y:50}, {x:35,y:35},
  ];
  ...
  batch.forEach((item, i) => {
    ...
    const entity = makeEntity(
      eid,
      Math.max(6, Math.min(COLS-6, pos.x + Math.floor((Math.random()-.5)*6))),
      Math.max(6, Math.min(ROWS-6, pos.y + Math.floor((Math.random()-.5)*6))),
  ...
  // Start after brief countdown (3s)
  setTimeout(() => startRoom(room), 3000);
  console.log(`[Room ${room.id}] Created with ${batch.length} players`);
}
