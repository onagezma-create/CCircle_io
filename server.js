...
function getPlayerSpawnPositions(count) {
  // Place players far apart using golden ratio spiral
  const positions = [];
  const minDist = 25; // minimum distance between spawns
  const candidates = [
  ...
  for (const c of candidates) {
    ...
    const tooClose = positions.some(p => Math.abs(p.x-c.x)+Math.abs(p.y-c.y) < minDist);
    if (!tooClose) positions.push(c);
  }
  while (positions.length < count) {
    positions.push({x: 10+Math.floor(Math.random()*80), y: 10+Math.floor(Math.random()*80)});
  }
  return positions;
}
...
function gameTick(room) {
  ...
  // Auto-end: if only 1 or 0 entities alive, end round
...
function setupPartyEvents(socket) {
  ...
  socket.on('party:start', ({ code }) => {
    const room = partyRooms.get(code);
    if (!room) return;
    if (room.hostSocketId !== socket.id) return; // only host can start

    // Put all party members into matchmaking queue (priority group)
    const members = room.players.map(p => ({
      socketId: p.socketId,
      playerData: { name: p.name, color: '#60a5fa', trailColor: '#1d4ed8' },
    }));

    // Get actual socket data for each member (color/skin)
    members.forEach(m => {
      const s = io.sockets.sockets.get(m.socketId);
      if (s && s.data.pendingPlayerData) m.playerData = s.data.pendingPlayerData;
    });

    // Create a dedicated room for the party
    ...
    // Notify party to start game
    io.to('party_' + code).emit('party:start_game');

    setTimeout(() => startRoom(gameRoom), 5000);
    console.log(`[Party] ${code} started → Room ${gameRoom.id}`);
  });
}
