...
function onCellEnter(e, room) {
  ...
  if (trailOwner >= 0) {
    if (trailOwner === e.id) {
      ...
      // Stepped on someone's trail → OWNER of trail dies
      const owner = findEntityById(room, trailOwner); // must use id-based lookup
...
function setupPartyEvents(socket) {
  ...
  socket.on('party:start', ({ code, withBots }) => {
    ...
    io.to('party_' + code).emit('party:start_game', { roomId: gameRoom.id });

    setTimeout(() => startRoom(gameRoom), 5000);
    console.log(`[Party] ${code} started → Room ${gameRoom.id} bots=${withBots}`);
  });
}
